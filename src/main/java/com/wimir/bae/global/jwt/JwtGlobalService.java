...
public class JwtGlobalService {
    ...
    public UserLoginDTO getTokenInfo(String accessToken, String  targetUserClass) {
        ...
        if(accessToken == null || accessToken.isEmpty())
            throw new IllegalArgumentException("토큰 값이 필요합니다.");
        ...
        Date now = new Date();
        if(now.after(tokenDate)) throw new IllegalArgumentException("세션이 만료되었습니다. 계속하려면 다시 로그인하세요.");
        ...
        if(userClass == null) throw new IllegalArgumentException("존재하지 않는 사원입니다.");

        switch(targetUserClass) {
            case "R":
                if(!userClass.equals(targetUserClass)) throw new RuntimeException("접근 권한이 없습니다.");
                break;
            case "A":
                if(!List.of("R","A").contains(userClass)) throw new RuntimeException("접근 권한이 없습니다.");
                break;
            case "U":
                if(!List.of("R","A","U").contains(userClass)) throw new RuntimeException("접근 권한이 없습니다.");
                break;
            default:
